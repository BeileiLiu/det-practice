...
> 状态：2026 改版第 3 轮（P0 完善度 + P1 架构拆分：引擎/数据层/AI 客户端全部出仓）后归档。本文描述当前代码如何组织，以及剩余演进路线。

## 1. 总体形态

- **单一入口 + 三个独立可测模块**：`index.html`（约 3900 行）含 HTML + `<style>` + 内联 JS（题型渲染、导航、考试流程、AI 生成编排）；**`js/engine.js`**＝评分/自适应/听写纯函数；**`js/storage.js`**＝9 个 localStorage key 的读写封装 + schema 版本化（AppStorage）；**`js/ai.js`**＝AI 生成模板/结构校验/OpenAI 兼容调用（AppAI）；**`server.py`**＝可选本地代理+静态服务；**`tests/`**＝三个测试文件 39 用例；**`smoke.js`**＝38 检查点整站冒烟；**`fonts/`**＝Nunito 本地字重（离线可用）；**`README.md`**＝用户/开发者快速上手。仍零构建、零依赖；`file://` 双击可用。
- **零构建、零依赖、零测试**；`file://` 双击可用（移动端抽屉、localStorage 均 OK；摄像头功能需 `http://localhost`）。
- 不是 git 仓库；兄弟文件：`images/`（11 张看图题照片）、`_det_sample.pdf`/`arno-19-types.md`（题库设计参考）、`index.backup-before-codex-20260801.html`（只读快照）、`_old_block.txt`（旧视线检测代码片段）、`docs/`（本文档与前端交接规格）。
